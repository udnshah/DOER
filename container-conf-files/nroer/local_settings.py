...
#    We have 2 database (postgres and sqlite3) connection details here. Ensure the part of connection details of database not in use is commented.  
        ...
        'NAME': 'oer',						# Used for postgres db 
        'USER': 'oer_db_user',							# Used for postgres db 
        'PASSWORD':'aiglab2homah',						# Used for postgres db 
        ...
        #'NAME': 'studio-nroer.db',
        ##'NAME': os.path.join(os.path.dirname(__file__),'studio-nroer.db'),
        #'NAME': os.path.join(os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe()))),os.path.dirname("../"),'studio-nroer.db'),
#    'NAME': os.path.join(GSTUDIO_DATA_ROOT, SQLITE3_DBNAME),		# Used for sqlite3 db          
    },
    'mongodb': {
        'ENGINE': 'django_mongokit.mongodb',				# Used for mongo db 
        'NAME': 'nroer-mongokit',					# Used for mongo db 
